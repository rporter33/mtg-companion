...
export default function useEngineCheck({ address, decks, first = null }) {
  ...
  const idsKey = address ? [...new Set(decks.flatMap((d) => (d.main ?? []).map((e) => e.cardId)))].sort().join('\n') : ''
  useEffect(() => {
  ...
  const askKey = address && seats.size
    ? `${address}|${first ?? ''}|${[...seats].map(([id, s]) => `${id}=${JSON.stringify(s.deck)}`).join('|')}`
    : ''
  useEffect(() => {
    ...
    ;(async () => {
      for (const id of ids) {
        ...
        const list = JSON.stringify(seat.deck)
        let found = answers.current.get(list)
        if (!found) {
          try {
            found = { reply: await api.check(seat.deck, { signal: ctl.signal }) }
  ...
  return useMemo(() => {
    ...
    for (const deck of decks) {
      ...
      const done = seat && results.get(`${deck.id}=${JSON.stringify(seat.deck)}`)
