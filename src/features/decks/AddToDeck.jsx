...
import { getFormat, formatLabel, canBeCommander, legalityStatus } from '../../lib/formats.js'
...
export default function AddToDeck({ card }) {
  ...
  return (
    <div className="stack stack--snug">
      ...
      {decks.map((deck) => {
        // A format this build does not know has no rules here (see getFormat):
        // the deck takes the card, and the row says nothing was checked rather
        // than calling the card legal or not.
        const format = getFormat(deck.formatId)
        const label = formatLabel(deck.formatId)
        ...
        return (
          <div className="row stack--tight" key={deck.id}>
            <button
              ...
              onClick={() => add(deck)}
              title={!format ? `Not checked: this version of the app does not know this deck's format, so it has no rules to check ${card.name} against.`
                : blocked ? `${card.name} is not legal in ${label}`
                  : status === 'future_legal' ? `Not out yet. Scryfall's Future Standard lists ${card.name}.`
                    : notOut ? `Not out yet. Scryfall sets its ${label} legality at release.`
                      : unknown ? `Came out on ${releaseLabel(card.released_at)}. The Scryfall data on this device lists it as legal in no format, so the app does not know whether it is ${label}-legal.`
                        : undefined}
            >
              {deck.name}
              <span className="faint tiny" style={{ marginLeft: 6 }}>{label}</span>
              ...
              {!format && <span className="chip chip--warn tiny ml-auto">format not known</span>}
            </button>
            {commanderOk && (
