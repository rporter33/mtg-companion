...
const IS_RAMP = (card) => {
  ...
  return /\badd \{/.test(text)
    || /\bsearch your library for a[n]? (basic )?land\b/.test(text)
    || /\bput a[n]? .*land.* onto the battlefield\b/.test(text)
}

const IS_CREATURE = (card) => /\bCreature\b/.test(typeLineOf(card))
...
export function coachDeck(deck, lookup) {
  ...
  const removalTarget = commander ? 10 : 8
  checks.push(check({
    id: 'removal',
    label: 'Answers',
    have: removal,
  ...
  const drawTarget = commander ? 10 : 6
  checks.push(check({
    id: 'draw',
    label: 'Card draw',
    have: draw,
  ...
  if (commander) {
    ...
    checks.push(check({
      ...
      want: 10,
      unit: 'cards',
      severity: ramp >= 10 ? 'ok' : ramp >= 5 ? 'warn' : 'error',
      message: ramp >= 10
        ? `${ramp} cards that accelerate your mana.`
        : `${ramp} ramp card${ramp === 1 ? '' : 's'}. Commander decks usually want around 10.`,
      why: 'Commander decks are expensive and start at 40 life, so the game rewards getting ahead on mana early.',
      query: ramp < 10 ? `${scope} (o:"add {" or o:"search your library for a basic land") -t:land` : null,
    }))
  }

  // --- creatures ---------------------------------------------------------
  const creatures = countMatching(resolved, IS_CREATURE)
  const creatureFloor = commander ? 20 : 12
  checks.push(check({
    id: 'creatures',
    label: 'Creatures',
    have: creatures,
