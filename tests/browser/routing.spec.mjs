...
console.log('\nPlaces inside Learn')
await page.getByRole('button', { name: /Never played a card game/ }).click()
await page.waitForTimeout(300)
check('a track has an address', (await hash(page)) === '#/guide/track/beginner', await hash(page))
await page.getByRole('button', { name: /How you actually win/ }).click()
await page.waitForTimeout(300)
check('a lesson opened from a track keeps the track in its address', (await hash(page)) === '#/guide/track/beginner/goal', await hash(page))
await page.reload({ waitUntil: 'networkidle' })
await page.waitForTimeout(600)
check('a reload keeps the lesson open', (await page.locator('h1').first().innerText()) === 'How you actually win', await page.locator('h1').first().innerText())
check('and the lesson cannot be finished before its question is answered',
  (await page.getByRole('button', { name: 'Mark complete' }).count()) === 0 && /Answer the question to finish/.test(await page.locator('main').innerText()))
await page.getByRole('button', { name: /Nothing — you cannot lose from an empty library/ }).click()
await page.waitForTimeout(200)
check('a wrong answer explains itself and still withholds completion',
  /Not quite/.test(await page.locator('main').innerText()) && (await page.getByRole('button', { name: 'Mark complete' }).count()) === 0)
await page.getByRole('button', { name: 'Try again' }).click()
await page.getByRole('button', { name: /They draw from an empty library and lose/ }).click()
await page.waitForTimeout(200)
check('the right answer unlocks it', (await page.getByRole('button', { name: 'Mark complete' }).count()) === 1)
await page.getByRole('button', { name: 'Mark complete' }).click()
await page.waitForTimeout(300)
check('finishing returns to the track, which shows the tick', (await hash(page)) === '#/guide/track/beginner' && /✓/.test(await page.locator('main').innerText()), await hash(page))
await page.goBack()
await page.waitForTimeout(300)
check('Back retraces into the lesson', (await hash(page)) === '#/guide/track/beginner/goal', await hash(page))
await page.goto(`${TARGET}#/guide/lesson/nonsense`, { waitUntil: 'networkidle' })
await page.waitForTimeout(500)
check('an unknown lesson shows a way back rather than a blank screen', /no lesson at this address/.test(await page.locator('main').innerText()))
await page.goto(`${TARGET}#/guide/game`, { waitUntil: 'networkidle' })
await page.waitForTimeout(600)
check('the guided game has an address', (await page.locator('.tutorial').count()) === 1)
await page.getByRole('button', { name: 'Next' }).click()
await page.waitForTimeout(200)
await page.reload({ waitUntil: 'networkidle' })
await page.waitForTimeout(600)
check('a reload keeps the game and its place', (await page.locator('.tutorial').count()) === 1 && /2 of \d+/.test(await page.locator('.tutorial .faint.tiny').first().innerText()),
  await page.locator('.tutorial .faint.tiny').first().innerText())
await page.getByRole('button', { name: /Leave/ }).click()
await page.waitForTimeout(300)
check('leaving lands on Learn', (await hash(page)) === '#/guide', await hash(page))
